...
// Human-readable names for standardized GATT characteristics, pulled from:
...
fileprivate let characteristicUuids: [String: String] = [
...
func resolveServiceUuid(for name: String) -> UUID? {
    UUID.init(uuidString: serviceUuids[name] ?? name)
}

func resolveCharacteristicUuid(for name: String) -> UUID? {
    UUID.init(uuidString: characteristicUuids[name] ?? name)
}