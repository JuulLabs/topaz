...
const appLog = function (msg) {
    return window.webkit.messageHandlers.logging.postMessage({
        msg: msg
    }).catch(rethrowAsDOMException);
};
...
class Store {
    constructor() {
        ...
        this.getDevice = (uuid) => {
            ...
            const d = __classPrivateFieldGet(this, _Store_devices, "f");
            appLog('getDevice 1 ' + d);
            const d1 = __classPrivateFieldGet(this, _Store_devices, "f").get(uuid);
            appLog('getDevice 2 ' + d1);
            const d2 = (_a = __classPrivateFieldGet(this, _Store_devices, "f").get(uuid)) === null || _a === void 0 ? void 0 : _a.device;
            if (d2) {
            ...
            return (_b = __classPrivateFieldGet(this, _Store_devices, "f").get(uuid)) === null || _b === void 0 ? void 0 : _b.device;
        };
...
class BluetoothLEScan {
    constructor(scanId, filters, keepRepeatedDevices, acceptAllAdvertisements, active) {
        ...
        this.active = active;
        this.stop = () => {
            bluetoothRequest('requestLEScan', { scanId: this.scanId, stop: true }).then(() => {
                this.active = false;
            });
        };
    }
...
const doRequestLEScan = async (options) => {
    ...
    return new BluetoothLEScan(response.scanId, response.filters, response.keepRepeatedDevices, response.acceptAllAdvertisements, response.active);
};
...
const processEvent = (event) => {
    ...
    if (event.name === 'gattserverdisconnected') {
        ...
        for (const [key, value] of Object.entries(adEvent.advertisement.serviceData)) {
            appLog('service data item: ' + key + ' = ' + value);
