...
public class AppModel {
    ...
    let appMessageProcessor: AppMesssageProcessor
    ...
    public init(
        ...
        appMessageProcessor: AppMesssageProcessor,
    ...
    private func loadWebContainerModel(tab: Int, url: URL, navBarModel: NavBarModel) async -> WebContainerModel? {
        do {
            ...
            await appMessageProcessor.setOnUserAgentModeChange { [webPageModel = model.webPageModel] mode in
                await webPageModel.setUserAgentMode(mode)
            }
            return model
        } catch {
