...
struct ReadDescriptorTests {
    ...
    func execute_withSuccessfulReadEvent_respondsWithData() async throws {
        ...
        let client = clientThatSucceeds(with: expectedData, eventBus: eventBus)
    ...
    func execute_withDescriptorBlockedForWriting_doesNotThrow() async throws {
        ...
        let client = clientThatSucceeds(with: Data(), eventBus: eventBus)
    ...
    private func clientThatSucceeds(with data: Data, eventBus: EventBus) -> BluetoothClient {
        var client = MockBluetoothClient()
        client.onDescriptorRead = { peripheral, descriptor in
