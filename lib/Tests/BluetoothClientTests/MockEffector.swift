...
struct MockEffector: BluetoothEffector {
    ...
    var mockRun: @Sendable (Message.Action, UUID, @Sendable (RequestClient) throws -> Void) async throws -> Void

    init(
        client: RequestClient = .testValue,
        mockReady: @escaping @Sendable () throws -> Void = { fatalError("Not implemented") },
        mockRun: (@Sendable (Message.Action, UUID, @Sendable (RequestClient) throws -> Void) async throws -> Void)? = nil
    ) {
        self.client = client
        self.mockReady = mockReady
        self.mockRun = mockRun ?? { _, _, effect in try effect(client) }
    }
